...
class TestFrame extends JFrame {
    ...
    public TestFrame(String name) {
        ...
        Image image = icon.getImage();//image=tool.getImage(Test_GUI.class.getResource("D:\\java\\icon\\tubiao.jpg"));
        setIconImage(image);
        setResizable(false);
        //------------菜单条的设置---------------
        mb = new JMenuBar();
        ...
        menuhelp.add(about);
        //设置快捷键
        calculator.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.CTRL_MASK));
        edition.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_MASK));
        ...
        edition.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                JOptionPane.showMessageDialog(null, "单机版", "版本信息", JOptionPane.PLAIN_MESSAGE);
            }
        });
        ...
        menuexit.addMouseListener(new MouseListener() {
            public void mouseClicked(MouseEvent arg0) {
                int temp = JOptionPane.showConfirmDialog(null, "您确认要退出系统吗？", "确认对话框", JOptionPane.YES_NO_OPTION);
...
class TestPanel extends JPanel implements ActionListener {
    private JLabel totaltime, lefttime, ttimeshow, ltimeshow, textinfo, userinfo;
    private JLabel copyright;
    private JButton starttest, back, next, commit;
    private JTextArea area;
    private JRadioButton rbtna, rbtnb, rbtnc, rbtnd;
    private String totaltimer = "", lefttimer = "", username = "", select = "";
    ...
    public TestPanel(String name) {
        ...
        area.setText("考场规则：\n" + "一、考试前15分钟，凭准考证和         身份证进入考场，对号入座，将准考证和" + "身份证放在桌面右上角，         便于监考人员检查.\n" + "二、笔译考试开考30分钟后不得入场，答         题结束后提交试卷后可以申请离场.\n" + "三、考生要爱惜考场的机器         和相关设备，严格按照固定的操作说明进行操作，如有人为损坏，照         价赔偿.");
    ...
    public void ReadTestquestion() {
        try {
            ...
            if (file.isFile() && file.exists()) { //判断文件是否存在
                ...
                total = s.length;
                textinfo.setText("第" + current + "题" + ",共" + total + "题");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == starttest) {
            JOptionPane.showMessageDialog(null, "请选择考试文件 ", "消息框", JOptionPane.PLAIN_MESSAGE);
            testTime();
    ...
    public void scorereport() {
        ...
        JOptionPane.showMessageDialog(null, "题   目  总   计: " + total + "\n正 确 的 题 目:   " + number + "\n成绩：  " + score, "考试成绩", JOptionPane.PLAIN_MESSAGE);
    }
}
